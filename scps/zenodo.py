...
def vintage_metadata(dep: VintageDeposit, repo: str = "seandavi/state-cancer-profile-scraper") -> dict:
    ...
    description = (
        ...
        f"<p><b>What a vintage is:</b> one edition of the upstream estimates — "
        f"the complete set of values the State Cancer Profiles website served "
        f"during some period. When NCI updates the underlying data (a new "
        f"SEER/NPCR submission, revised population denominators, a shifted "
        f"five-year window), the site silently replaces the old values; that "
        f"replacement marks the boundary between one vintage and the next. "
        f"Several dated scrapes that captured identical values belong to one "
        f"vintage, so each version of this record preserves one edition of "
        f"the estimates that is no longer available from the source. (The "
        f"term follows the real-time data literature in economics, where a "
        f"vintage is a statistical series as it existed on a given date.)</p>"
